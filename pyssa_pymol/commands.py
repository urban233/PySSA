...
from pyssa_pymol import local_logging
...
def scene(a_key: str, an_action: str) -> tuple[bool, str]:
    ...
    if an_action == "rename":
        ...
        return False, "an_action cannot be rename because a new scene name cannot be defined."
    if an_action not in ["store", "recall", "insert_after", "insert_before", "next", "previous", "update", "rename", "clear", "append"]:
...
def show_custom_representation(a_representation: str, a_selection_string: str) -> tuple[bool, str]:
    ...
    if a_representation not in constants.PYMOL_REPS_WITH_INDICES.values():
        ...
        return False, "a_representation is not found in the PYMOL_REPS_WITH_INDICES dict."
    if a_selection_string is None or a_selection_string == "":
        logger.error("a_selection_string is either None or an empty string.")
        return False, "a_selection_string is either None or an empty string."
...
def zoom_with_custom_parameters(a_selection_string: str,
    ...
    if a_complete_flag is None or a_complete_flag < 0 or a_complete_flag > 1:
        ...
        return False, "a_complete_flag is either None or invalid (it can only take 0 or 1)."
...
def ray(a_width: int, a_height: int, a_renderer: int) -> tuple[bool, str]:
    ...
    if a_renderer is None or a_renderer < 0 or a_renderer > 3:
        ...
        return False, "a_renderer is either None or is invalid (only 0 - 3 is valid)"
...
def draw(a_width: int, a_height: int, an_antialias_value: int) -> tuple[bool, str]:
    ...
    if an_antialias_value is None or an_antialias_value < 0:
        ...
        return False, "an_antialias_value is either None or has a value less than 0."
