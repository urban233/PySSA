...
class Protein:
  ...
  def add_protein_structure_data_from_pdb_db(
    ...
    tmp_pdb_filepath = pathlib.Path(
        f"{constants.CACHE_PROTEIN_DIR}/{a_pdb_id}.pdb"
    )
  ...
  def set_id(self, a_value: int) -> None:
    ...
    if a_value is None or a_value < 0:
      ...
      raise exception.IllegalArgumentError(
          "a_value is either None or has a value less than 0."
      )
  ...
  def get_protein_sequences(self) -> list["sequence.Sequence"]:
    ...
    Returns:
        A list of sequence objects.
    """
    return protein_operations.get_protein_sequences_from_protein(
        self._pymol_molecule_object, self.chains
    )

  def get_sequences_based_on_chain_letter(
      self, chain_letter: str
  ...
  def write_fasta_file(self, a_filepath: pathlib.Path) -> None:
    ...
    try:
      fasta_file: TextIO = open(
          f"{a_filepath}/{self._pymol_molecule_object}.fasta", "w"
      )
    except OSError:
    ...
    logger.info(
        f"Fasta file for sequence {self._pymol_molecule_object} written."
    )
    fasta_file.close()
  ...
  def append_chain(
    ...
    if chain_type is None or chain_type == "":
      ...
      raise exception.IllegalArgumentError(
          "chain_type is either None or an empty string."
      )
  ...
  def get_chain_by_letter(self, a_letter: str) -> Optional["chain.Chain"]:
    ...
    if a_letter is None or a_letter == "":
      ...
      raise exception.IllegalArgumentError(
          "a_letter is either None or an empty string."
      )
