...
class PredictionAnalysisWorkerPool(QtCore.QRunnable):
    """This class is a worker class for the analysis process.
    ...
    def __init__(self,
                 ...
                 _init_batch_analysis_page,
                 table_prot_to_predict: QtWidgets.QTableWidget,
                 prediction_config: prediction_configuration.PredictionConfiguration) -> None:
        """Constructor.
        ...
        Raises:
            ValueError: raised if an argument is illegal
        """
        super(PredictionAnalysisWorkerPool, self).__init__()
        self.table = table_prot_to_predict
        self.prediction_configuration = prediction_config
        self.list_analysis_overview = list_analysis_overview
        self.cb_analysis_images = cb_analysis_images
        ...
        self.prediction_sub_worker = PredictionWorkerPool(self.table, self.prediction_configuration, self.app_project)
        self.analysis_sub_worker = AnalysisWorkerPool(self.list_analysis_overview, self.cb_analysis_images,
                                                      self.status_bar, self.app_project, self.app_settings,
                                                      self._init_batch_analysis_page)

    def run(self):
        self.prediction_sub_worker.run()
        self.analysis_sub_worker.run()
        self.signals.finished.emit()
