...
#
"""Module contains all enums used in PySSA."""
import enum
...
class SQLQueryStatement(enum.Enum):
  ...
  DELETE_SEQUENCE = """
    ...
    """
  GET_SEQUENCES = (
      """SELECT seq_id, seq, name FROM SeqRecord WHERE project_id = ?"""
  )
  ...
  GET_PYMOL_SELECTIONS = (
      """SELECT selection_string FROM PyMOLSelection WHERE protein_id = ?"""
  )
  GET_CHAINS = """SELECT id, chain_identifier, chain_type, chain_sequence FROM Chain WHERE protein_id = ?"""
  GET_PYMOL_PARAMETERS = (
      """SELECT color, representation FROM PyMOLParameter WHERE chain_id = ?"""
  )
  GET_PROTEIN_NAME_BY_ID = (
      """SELECT pymol_molecule_object FROM Protein WHERE id = ?"""
  )
