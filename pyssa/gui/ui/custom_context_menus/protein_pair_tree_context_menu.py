...
class ProteinPairTreeContextMenu:
  ...
  def __init__(self) -> None:
    ...
    self._collapse_protein_pair_action = QtWidgets.QAction(
        "Collapse Protein Pair"
    )
    self._open_results_summary_action = QtWidgets.QAction(
  ...
  def connect_expand_protein_pair_action(
  ...
  def connect_collapse_protein_pair_action(
      self, the_function_to_connect: Callable
  ) -> None:
    """Connects the `_expand_protein_pair_action` triggered signal to a given callable.
    ...
    self._collapse_protein_pair_action.triggered.connect(
        the_function_to_connect
    )

  def connect_open_results_summary_action(
      self, the_function_to_connect: Callable
  ) -> None:
  ...
  def connect_color_based_on_rmsd_action(
      self, the_function_to_connect: Callable
  ) -> None:
  ...
  def get_context_menu(
    ...
    if is_protein_pair_in_current_session_flag is None:
      ...
      raise exception.IllegalArgumentError(
          "is_protein_pair_in_current_session_flag is None."
      )
