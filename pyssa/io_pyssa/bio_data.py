...
def convert_pdb_data_list_to_pdb_file(
    a_pdb_filepath: pathlib.Path, a_pdb_data: list
) -> None:
...
def build_pdb_file(records: list, a_filepath: str) -> None:
  ...
  if records is None or len(records) == 0:
    ...
    raise exception.IllegalArgumentError(
        'records is either None or an empty list.'
    )
  ...
  with open(a_filepath, 'w') as pdb_file:
    for record in records:
      ...
      if record_type == 'ATOM':
        pdb_line = (
            f"{record_type:6s}{record['atom_number']:5d}  {record['atom_name']:4s}"
        ...
        try:
          pdb_line = (
              f"{record_type:3s}   {record['atom_number']:5d}      {record['residue_name']:3s}"
