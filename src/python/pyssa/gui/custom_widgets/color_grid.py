...
class ColorGrid(QtWidgets.QWidget):
  """Class representing a color grid."""
  ...
  def generate_color_stylesheet(self, a_hex_color: str) -> str:
    ...
    Raises:
        exception.NoneValueError: If `a_hex_color` is None.
        exception.IllegalArgumentError: If `a_hex_color` is an empty string.
    """
    # <editor-fold desc="Checks">
    if a_hex_color is None:
      default_logging.append_to_log_file(logger, "a_hex_color is None.", logging.ERROR)
      raise exception.NoneValueError("a_hex_color is None.")
    if a_hex_color == "":
      default_logging.append_to_log_file(logger, "a_hex_color is an empty string.", logging.ERROR)
      raise exception.IllegalArgumentError("a_hex_color is an empty string.")
    # </editor-fold>
  ...
  def set_icon_for_selected_color(self, a_color: str) -> None:
    """DEPRECATED! Sets the icon for the button with the given color.
    ...
    if a_color is None or a_color == "":
      default_logging.append_to_log_file(logger, "a_color is either None or an empty string.")
