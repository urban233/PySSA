...
from pyssa.gui.custom_widgets import custom_label
...
class StatusBarManager:
  ...
  def __init__(self, the_main_view: "main_frame.MainFrame") -> None:
    """Constructor.
    ...
    # </editor-fold>
    self._view: "main_frame.MainFrame" = the_main_view
    ...
    self._view.ui.statusbar.addPermanentWidget(self._progress_bar)
    self._view.ui.statusbar.addPermanentWidget(self._permanent_message)
    self._view.ui.statusbar.addPermanentWidget(self._view.btn_open_job_overview)
    self._view.ui.statusbar.addPermanentWidget(
  ...
  def _style_status_bar_for_normal_message(self) -> None:
    ...
    self._view.ui.statusbar.setStyleSheet(
        """
            QStatusBar {
  ...
  def show_error_message(
    ...
    if overwrite_permanent_message is True:
      ...
      self._view.ui.statusbar.showMessage(a_message, 999999)

  def show_temporary_message(
      self,
      a_temporary_message: str,
      a_with_timeout_flag: bool = True,
      a_timeout: int = model_definitions.ModelDefinitions.STATUS_MESSAGE_TIMEOUT,
  ) -> None:
    ...
    if a_with_timeout_flag:
      self._view.ui.statusbar.showMessage(a_temporary_message, a_timeout)
    else:
      self._view.ui.statusbar.showMessage(a_temporary_message, 999999)
