...
from src.pyssa.gui.ui.forms.auto_generated import auto_dialog_welcome_screen
from src.pyssa.gui.ui.styles import styles
from src.pyssa.util import constants, input_validator


class WelcomeScreenView(QtWidgets.QDialog):
  """Class representing the welcome screen dialog."""
  ...
  def __init__(self, parent=None) -> None:
    ...
    # build ui object
    self.ui = auto_dialog_welcome_screen.Ui_Dialog()
  ...
  def _initialize_ui(self) -> None:
    ...
    # TODO: Fix the blurry PySSA logo
    scaled_pixmap = original_pixmap.scaled(48, 48, transformMode=QtCore.Qt.TransformationMode.SmoothTransformation)
    self.ui.lbl_pyssa_icon.setPixmap(scaled_pixmap)
    self.ui.lbl_welcome.setStyleSheet("""QLabel { font-size: 16px; font-weight: bold; }""")
    self.setWindowIcon(QtGui.QIcon(constants.PLUGIN_LOGO_FILEPATH))
    self.setWindowTitle(f"PySSA {constants.VERSION_NUMBER.replace('v', '')}")
    self.setWindowFlags(
