...
class CreateProjectViewController(QtCore.QObject):
  ...
  def _restore_ui(self) -> None:
    ...
    self._view.ui.lbl_new_status_choose_reference.setStyleSheet(
        "color: #ba1a1a; font-size: 11px;"
    )
    self._view.ui.lbl_new_status_project_name.setText("")
    self._view.ui.lbl_new_status_project_name.setStyleSheet(
        "color: #ba1a1a; font-size: 11px;"
    )
  ...
  def _convert_model_into_set(self) -> set:
    ...
    for tmp_row in range(
        self._view.ui.list_create_projects_view.model().rowCount()
    ):
  ...
  def _connect_all_ui_elements_to_slot_functions(self) -> None:
    ...
    self._view.ui.txt_new_project_name.textChanged.connect(
        self._validate_project_name
    )
    self._view.ui.cb_new_add_reference.clicked.connect(
        self._show_add_protein_options
    )
    self._view.ui.btn_new_choose_reference.clicked.connect(
        self._load_reference_in_project
    )
    self._view.ui.btn_new_create_project.clicked.connect(
        self._create_new_project
    )
    self._view.ui.txt_new_choose_reference.textChanged.connect(
        self._validate_reference_in_project
    )
