...
from src.pyssa.controller import database_manager, pymol_session_manager, interface_manager
...
def load_protein_pymol_session(
  ...
  if needs_to_be_reinitialized_flag:
    logger.info(
        "The current session is not empty. Reinitialize session now ..."
    )
    the_pymol_session_manager.reinitialize_session()
  ...
  try:
    ...
    logger.error(
        f"Loading the session failed because this error was raised: {e}"
    )
    return the_pymol_session_manager, False
  else:
  ...
  try:
    with database_manager.DatabaseManager(
      ...
      tmp_protein.pymol_session = (
          the_interface_manager.pymol_session_manager.save_current_session_as_base64()
      )
...
def save_protein_pair_pymol_session_to_database(
  ...
  try:
    with database_manager.DatabaseManager(
      ...
      tmp_protein_pair = (
          the_interface_manager.get_current_active_protein_pair_object()
      )
...
def load_scene(
  ...
  time.sleep(
      0.5
  )  # This should increase the stability because the scene command has more time in between command calls
  try:
...
def delete_scene(
  ...
  try:
    the_pymol_session_manager.user_pymol_connector.scene(
        a_key=the_pymol_session_manager.current_scene_name,
        an_action="clear",
    )
  except Exception as e:
    logger.error(f"Unexpected error occurred. Exception: {e}")
...
def reset_color_pymol_selection_atoms_by_element(
  ...
  if a_chain_letter is None or a_chain_letter == "":
    ...
    return False, ""
  if (
      the_current_active_chain_color_of_protein is None
      or the_current_active_chain_color_of_protein == ""
  ):
    logger.error(
        "the_current_active_chain_color_of_protein is either None or an empty string."
    )
    return False, ""
  if a_pymol_selection is None or a_pymol_selection == "":
    logger.error("a_pymol_selection is either None or an empty string.")
  ...
  try:
    ...
    the_pymol_session_manager.color_protein(
        tmp_chain_color, f"{a_pymol_selection}"
    )
  except Exception as e:
    logger.error(f"Unexpected error occurred. Exception: {e}")
    return False, ""
...
def color_pymol_selection_atoms_by_element(
  ...
  try:
    the_pymol_session_manager.color_protein(
        "atomic",
        f"{a_pymol_selection} and not elem C",
    )
    the_pymol_session_manager.color_protein(
        "grey70",
        f"{a_pymol_selection} and elem C",
    )
  except Exception as e:
    logger.error(f"Unexpected error occurred. Exception: {e}")
    return (False,)
...
def set_background_color(
  ...
  try:
    the_pymol_session_manager.user_pymol_connector.set_background_color(
        a_background_color
    )
  except Exception as e:
    logger.error(f"Unexpected error occurred. Exception: {e}")
