...
#
"""Module contains the colabbatch class."""
import os
import logging
import subprocess
import zmq
...
from src.pyssa.util import constants, prediction_util
...
class Colabbatch:
  ...
  def __init__(
    ...
    if a_prediction_configuration.amber_force_field is None:
      ...
      raise exception.IllegalArgumentError(
          "a_prediction_configuration.amber_force_field is None."
      )
    if (
        a_prediction_configuration.templates is None
        or a_prediction_configuration.templates == ""
    ):
      logger.error(
          "a_prediction_configuration.templates is either None or an empty string."
      )
      raise exception.IllegalArgumentError(
          "a_prediction_configuration.templates is either None or an empty string."
      )
  ...
  def setup_prediction_service(self) -> None:
    ...
    try:
      if prediction_util.delete_pyssa_colabfold_directory_in_wsl2():
        logger.info(
            "The pyssa_colabfold directory has been successfully deleted."
        )
      else:
    ...
    for tmp_key in result.keys():
      ...
      if tmp_key == "error":
        ...
        raise exception.PredictionEndedWithError(
            f"Prediction run into the error: {result[tmp_key]}!"
        )
    ...
    try:
      ...
      if tmp_output:
        ...
        try:
          ...
          raise exception.PredictionEndedWithError(
              "Could not copy prediction results to Windows host!"
          )
        else:
          logger.info(
              "Copying prediction results to Windows host was successful."
          )
      else:
        logger.error(
            "Prediction finished with errors! No results were copied to the Windows host!"
        )
        raise exception.PredictionEndedWithError("")
    return
