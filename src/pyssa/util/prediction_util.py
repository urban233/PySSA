...
#
"""Module contains helper function for the prediction process."""
import os
import pathlib
import logging
import shutil
...
def get_prediction_name_and_seq_from_table(
  ...
  for tmp_prot_name, tmp_seqs in groups.items():
    prediction_runs.append(
        prediction_protein_info.PredictionProteinInfo(tmp_prot_name, tmp_seqs)
    )
...
def get_relaxed_rank_1_pdb_file(
  ...
  for tmp_protein_to_predict in proteins_to_predict:
    ...
    for tmp_filename in os.listdir(pathlib.Path(constants.PREDICTION_PDB_DIR)):
      if (
          tmp_filename.find(f"{tmp_protein_to_predict.name}_relaxed_rank_001")
          != -1
      ):
...
def copy_pyssa_colabfold_directory_to_wsl2() -> None:
  ...
  if not os.path.exists(tmp_pyssa_colabfold_windows_path):
    logger.error(
        f"The variable 'tmp_pyssa_colabfold_windows_path' is illegal: {tmp_pyssa_colabfold_windows_path}!"
    )
  ...
  try:
    ...
    shutil.copytree(
        tmp_pyssa_colabfold_windows_path, tmp_pyssa_colabfold_wsl_path
    )
    if os.path.exists(tmp_pyssa_colabfold_wsl_path):
